...
public class Deplace implements InterfaceConstantes{
	...
	public double[] getAnimRotationTir(AbstractModelPartie partie, boolean getForArrow)
	{
		...
		double ycenter= heros.getYpos()+(isFiring? ((Mouvement_entity)heros.getDeplacement()).y_center_tir.get(heros.getAnim()):
			(heros.getDeplacement().ytaille.get(heros.getAnim())/4))-6;//arms at neck level
